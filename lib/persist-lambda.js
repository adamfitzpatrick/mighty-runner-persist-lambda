...
const doPut = (config, payload) => {
  const Item = {
    [process.env.PRIMARY_KEY_COLUMN_NAME]: payload[process.env.PRIMARY_KEY_COLUMN_NAME],
    [process.env.SORT_KEY_COLUMN_NAME]: payload[process.env.SORT_KEY_COLUMN_NAME],
  ...
  return config.docClient.put(params).promise()
    ...
    .catch(err => Promise.reject(logResultMessage(config, Item, err)))
}
